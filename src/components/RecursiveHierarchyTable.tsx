...
import { Plus, Search, Eye, Edit, Trash2, Settings, Users as UsersIcon, ChevronLeft } from 'lucide-react';
...
interface RecursiveHierarchyTableProps {
  entityId?: string;
...
const RecursiveHierarchyTable: React.FC<RecursiveHierarchyTableProps> = ({ 
  entityId, 
  entityType,
  allowedEntityTypes,
  customerId,
  ...
  useEffect(() => {
    if (entityId) {
      ...
      fetchEntitiesByType(entityType);
    } else {
      fetchCustomers();
    }
  }, [entityId, entityType, customerId]);
  ...
  const fetchCustomers = async () => {
    try {
      ...
      // If not super admin, filter by customer
  ...
  const fetchEntitiesByType = async (type: EntityType) => {
    try {
      setLoading(true);
      let query = supabase
        .from('entities')
      ...
      // Filter by customer context if not super admin
      if (!isSuperAdmin() && customerId) {
        // We'd need to implement proper customer filtering here
        // This would require a recursive query or maintaining customer_id in entities
      }
  ...
  const fetchEntityDetails = async (id: string) => {
    try {
      ...
      const rootCustomerId = await findRootCustomer({ 
        ...entity, 
        entity_type: entity.entity_type as EntityType,
        metadata: entity.metadata || {},
        parent_id: entity.parent_id || null
      ...
      if (allowedEntityTypes && allowedEntityTypes.length > 0) {
        childQuery = childQuery.in('entity_type', allowedEntityTypes);
      }
      ...
      baseColumns.push({
        ...
        render: (_, row: Entity) => (
          <div className="flex items-center gap-2">
            ...
            {(isSuperAdmin() || userRole === 'customer_admin') && (
              <>
                <Button 
                  variant="ghost" 
                  size="sm"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {currentEntity && !readOnly && (isSuperAdmin() || userRole === 'customer_admin') && (
          <AddEntityDialog
...
export default RecursiveHierarchyTable;